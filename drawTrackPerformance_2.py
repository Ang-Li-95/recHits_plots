...
for det in ["BTL","ETL"]:
    ...
    histos[det+"cluster_energy_h"]=R.TH1F(det+"cluster_energy_h",det+"cluster_energy_h",1000,0.,10000.)
...
for ievent,event in enumerate(dh):
    ...
    clus_energy_totalval_BTL = 0   #the total value of recHit energy from BTL
    clus_energy_totalval_ETL = 0   #from ETL
    for itrack in range(0,len(event.track_idx)):
        matched_clus_n_BTL = 0
        matched_clus_n_ETL = 0
        if (not goodTrack(event,itrack,args.chi2cut)):
            continue
        if (event.track_eta_atBTL[itrack]>-100.):  #if the track is from BTL
            for irecHit in range(0,len(event.clusters_det)):   #loop over all recHits
                if (M.sqrt(pow((event.track_eta[itrack]-event.clusters_ieta[irecHit]),2)+pow(event.track_phi[itrack]-event.clusters_iphi[irecHit],2))<0.5):
                    clus_energy_totalval_BTL+=event.clusters_energy[irecHit]
                    matched_clus_n_BTL+=1
        else :
            for irecHit in range(0,len(event.clusters_det)):   #loop over all recHits
                if (M.sqrt(pow((event.track_eta[itrack]-event.clusters_ieta[irecHit]),2)+pow(event.track_phi[itrack]-event.clusters_iphi[irecHit],2))):
                    clus_energy_totalval_ETL+=event.clusters_energy[irecHit]
                    matched_clus_n_ETL+=1

        print "Number of matched clusters in the track(BTL): %d"%matched_clus_n_BTL
        print "Number of matched clusters in the track(ETL): %d"%matched_clus_n_ETL
    ...
    fill_clus_energy_BTL = clus_energy_totalval_BTL*1000.
    fill_clus_energy_ETL = clus_energy_totalval_ETL*1000.
    histos["BTLcluster_energy_h"].Fill(fill_clus_energy_BTL)
    histos["ETLcluster_energy_h"].Fill(fill_clus_energy_ETL)
